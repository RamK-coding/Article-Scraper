...
st.title("Scientific trends dashboard")

with st.sidebar.form("Parameters"):
    choice1 = st.selectbox("Choose to see trends for:", ("Sustainable Aviation Fuels", "--Power to liquid fuels", "Hydrogen Aviation", "Electric aviation", "--eVTOL","--UAM"))
...
if submit:
    if choice2 == "None":
        st.session_state.sc_choice = choice1
        dict = {"Sustainable Aviation Fuels": "sustainable aviation fuel",
                ...
                "Electric aviation": "electric aircraft",
                "--eVTOL": "eVTOL",
...
try:
    st.markdown("**:red[Top 10 most-cited publications]**")
    df = st.session_state.articles.sort_values("citations", ascending=False)[0:10].copy()
...
try:
    st.markdown("**:red[Cumulative article-count comparison]**")
...
try:
    st.markdown("**:red[Cumulative citations comparison]**")
